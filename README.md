...
수시로 너무 많은 api요청을 했기때문에 로딩과정이나 영화를 정렬하는 과정에서 버벅거림이 조금 있었다. 프로젝트가 끝나고 나서야 생각이 났는데, 제일 처음에 api요청을 하여 모든 영화들을 불러온뒤 state에 모든 영화를 젖아해놓고, 그 다음부터는 vue에서 filter를 써서 정렬이나 추천 등을 해줬으면 웹의 속도나 최적화 등에 더 효과적이지 않았을까 싶다.
그 외에도 이런저런 기능들을 더 많들 수 있었는데 못한게 아쉽고, 이미 만든것들도 디테일적인 부분에서 아쉬운것들이 많이 남는 첫번째 프로젝트였다!
